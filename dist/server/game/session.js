...
var Session=function(){ /**
   ...
   * @param io
   */function Session(io){_classCallCheck(this,Session);this._io=io;this._id=_shortid2.default.generate();this._store=(0,_store2.default)();this._gameData=this.loadGameData();this._isRunning=false;this._lastTickAt=null;this._entities=[];this._packetSequence=0;this._lastSyncAt=null; // Notify each client every time the state is changed.
this._unsubscribeFromStore=this._store.subscribe(this.handleStateChange.bind(this));_vendor.logger.info('session.create (session_id: '+this._id+')')} /**
   * Loads the game data for this game session.
   * @returns {Object}
   */_createClass(Session,[{key:'loadGameData',value:function loadGameData(){return {assets:require(_constants.DATA_PATH+'/assets.json'),config:require(_constants.DATA_PATH+'/config.json'),ui:require(_constants.DATA_PATH+'/ui.json'),map:(0,_map.createMap)('castle',this._store.dispatch)}} /**
   * Starts this session.
   ...
   */},{key:'update',value:function update(){var gameState=this.gameState;this.updateEntities(gameState)} /**
   * Ends this session.
   */},{key:'end',value:function end(){this._unsubscribeFromStore()} /**
   * Updates each entity in this session.
   */},{key:'updateEntities',value:function updateEntities(gameState){var _this=this;var removedEntityIds=this.getEntityIds();(0,_lodash.forEach)(gameState.entities,function(props){var entity=(0,_lodash.find)(_this._entities,function(e){return e.id===props.id}); // Create the entity if it does not exist.
...
this.destroyEntities(removedEntityIds)} /**
   ...
   * @param {Object} props
   */},{key:'addPlayer',value:function addPlayer(props){this._store.dispatch((0,_game.addEntity)(props));this._store.dispatch((0,_game.assignTeam)(props.id))} /**
   * Removes a player from this session.
   * @param {string} id
   */},{key:'removePlayer',value:function removePlayer(id){this._store.dispatch((0,_game.leaveTeam)(id));this._store.dispatch((0,_game.removeEntity)(id))} /**
